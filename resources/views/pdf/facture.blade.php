...
<header class="clearfix">
    ...
    <h1>Facture #{{str_pad($numFact, 6,'0',0)}}</h1>
    <div id="company">
...
<main>
    <table>
        ...
        @foreach($groupe_participants->participants as $participant)
            ...
            <tr>
                ...
                <td class="desc">
                                        ...
                                        @foreach($go->options_choisies as $option)
                                            <ul>
                                                <li>
                                                    ...
                                                    ?>
                                                    @if(!isset($option->prix_reel))
                                                        {{$option->option->nom.' '}} @if($option->option->prix) {{"(".$option->option->prix." €)"}}
                                                        ...
                                                        @endif
                                                    @else
                                                        {{$option->option->nom.' '}} @if($option->option->prix) {{"(".$option->prix_reel." €)"}}
                                                        @if($option->option->tva > 0)
                                                            (HT)
                                                            <?php
                                                            $pourcentageTVA += ($option->option->tva / 100) * $option->prix_reel
                    ...
                    @endforeach
                    <br/>
                    @if(sizeof($groupe_participants->vads) > 0)
                        <ul>
        ...
        <tr>
            ...
            <td colspan="4" style="background:none !important">Total HT</td>
            <td class="total" style="background:none !important">{{$totalMontant}} €</td>
        </tr>
        <tr>
            <td class="qty" style="background:none !important"></td>
            <td colspan="4" style="background:none !important">Dont TVA</td>
            <td class="total" style="background:none !important">{{$totalTVA}} €</td>
        </tr>
        <tr>
            <td class="qty" style="background:none !important"></td>
            <td colspan="4" style="background:none !important">Total TTC</td>
            <td class="total" style="background:none !important">{{$totalTTC = $totalMontant+$totalTVA+$a_deduire}}€
            </td>
        </tr>
        <tr>
            <td class="qty" style="background:none !important"></td>
            <td colspan="4" style="background:none !important">Réglé</td>
            <td class="total" style="background:none !important">{{$dejaRegle}} €</td>
        </tr>
        <tr>
            <td class="qty" style="background:none !important"></td>
            <td colspan="4" class="grand total" style="background:none !important; font-size:1.4em;">
                Reste à régler
            </td>
            @if($totalTTC == 0)
            ...
            @else
                ...
                @else
                    <td class="grand total"
                        style="background:none !important; font-size:1.4em;">{{$totalTTC-$dejaRegle}}
                        €
                    </td>
                @endif
            @endif
    ...
    <div class="row col-xs-12">
        ...
        <p><strong>Legal notice : </strong><br/>
            ...
            Intra-Community VAT : FR29435031943<br/>
            RC HISCOX - 19 RUE LOUIS LEGRAND - 75002 PARIS<br/>
            Financial guarantee : GROUPAMA <br/>
